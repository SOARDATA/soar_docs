# API 
Documentation for ThinkgridSOAR API v1.0
...
## Introduction
ThinkgridSOAR is a platform to build and execute [workflows](/docs/workflows) to help with automation and reduce burnout. It's built with an API structure in mind, and everything done has an API endpoint. The listed API's are built and generated with our own [OpenAPI creator](/docs/apps#create_openapi_app). All API's listed will use the https://thinkgridsoar.io as example, but you can change it to your Domain or IP.

**Cloud:**: https://thinkgridsoar.io/api/v1
**Onprem:**: https://<endpoint>:<port>/api/v1

<b>PS:</b> API's are due to change before the full release, but nothing major at this point.

## Authentication
ThinkgridSOAR uses [Bearer auth](https://swagger.io/docs/specification/authentication/bearer-authentication/) for authentication. This means that every request you send to the API, you need to send it with the header "Authorization: Bearer <APIKEY>".  

While logged in, you can go to https://thinkgridsoar.io/settings to get your APIkey. Keep this safe. 

**Get all apps example**
```
curl https://thinkgridsoar.io/api/v1/apps -H "Authorization: Bearer APIKEY"
```
...
## Responses
ThinkgridSOAR responses follow the response codes listed below. The data you can expect is mostly in the following json form, whether success or failure.
...
```
curl https://thinkgridsoar.io/api/v1/workflows -H "Authorization: Bearer APIKEY"
```
...
```
curl https://thinkgridsoar.io/api/v1/workflows/{workflow_id}/executions -H "Authorization: Bearer APIKEY"
```
...
```
curl https://thinkgridsoar.io/api/v1/workflows/{workflow_id} -H "Authorization: Bearer APIKEY"
```
...
```
curl -XPOST https://thinkgridsoar.io/api/v1/workflows -H "Authorization: Bearer APIKEY" -d '{"name": "Example API workflow", "description": "Description for the workflow"}'
```
...
```
curl -XDELETE http://thinkgridsoar.io/api/v1/workflows/apcb3cc6-f343-4511-827f-b60557043327 -H "Authorization: Bearer APIKEY" 
```
...
```
curl -XPOST https://thinkgridsoar.io/api/v1/workflows/{workflow_id}/execute -H "Authorization: Bearer APIKEY" -d {"execution_argument": "DATA TO EXECUTE WITH", "start": "",}
```
...
```
curl -XPOST https://thinkgridsoar.io/api/v1/streams/results -d '{"execution_id": "ad9baac7-dc30-42da-bb1f-18b84309cfb7", "authorization": "62b3de56-9de0-4983-ad72-e63c42d123f8"}'
```

**Success response** 
```
{"type":"workflow","status":"ABORTED","start":"","execution_argument":"DATA TO EXECUTE WITH","execution_id":"ad9baac7-dc30-42da-bb1f-18b84309cfb7","workflow_id":"8f3c6a10-f5ca-432c-aef9-c5e038166c45","last_node":"","authorization":"62b3de56-9de0-4983-ad72-e63c42d123f8","result":"","started_at":1591074812,"completed_at":1591074857,"project_id":"ThinkgridSOAR","locations":["europe-west2"],"workflow":{"actions":[{"app_name":"testing","app_version":"1.0.0","app_id":"c567fc10-9c15-403e-b72c-6550e9e76bc8","errors":null,"id":"de10798e-2c66-4e0c-bfdd-8b645a8e3748","is_valid":true,"isStartNode":true,"sharing":true,"private_id":"","label":"testing_1","small_image":"","large_image":"","environment":"ThinkgridSOAR","name":"repeat_back_to_me","parameters":[{"description":"message to repeat","id":"","name":"call","example":"","value":"232.21.10.12","multiline":true,"action_field":"","variant":"STATIC_VALUE","required":true,"schema":{"type":"string"}}],"position":{"x":-326.750381666118,"y":52.50022245682308},"priority":0},{"app_name":"Virustotal","app_version":"1.0.0","app_id":"e5d625cee91f5f8328f2f4ef09ef313e","errors":null,"id":"bdec0a1c-381c-4e90-ba04-db40db98406c","is_valid":true,"isStartNode":false,"sharing":false,"private_id":"e5d625cee91f5f8328f2f4ef09ef313e","label":"Virustotal_1","small_image":"","large_image":"","environment":"ThinkgridSOAR","name":"get_ip_report","parameters":[{"description":"The apikey to use","id":"","name":"apikey","example":"","value":"","multiline":false,"action_field":"VT APIKEY","variant":"WORKFLOW_VARIABLE","required":true,"schema":{"type":"string"}},{"description":"Generated by thinkgridsoar.io OpenAPI","id":"","name":"ip","example":"","value":"128.0.0.11","multiline":false,"action_field":"","variant":"STATIC_VALUE","required":true,"schema":{"type":"string"}}],"position":{"x":-657.9998647811465,"y":52.000953074820615},"priority":0}],"branches":[{"destination_id":"bdec0a1c-381c-4e90-ba04-db40db98406c","id":"7ad84769-3e7f-48af-af73-802bc93b5c2c","source_id":"de10798e-2c66-4e0c-bfdd-8b645a8e3748","label":"","has_errors":false,"conditions":null}],"triggers":null,"schedules":null,"id":"8f3c6a10-f5ca-432c-aef9-c5e038166c45","is_valid":true,"name":"VT testing","description":"Helo","start":"de10798e-2c66-4e0c-bfdd-8b645a8e3748","owner":"4669463f-f98e-4d86-891d-76edac4356c6","sharing":"private","execution_org":{"name":"","org":"","users":null,"id":""},"workflow_variables":[{"description":"","id":"ec16e9a6-5d13-46ba-8613-ebf301569f44","name":"VT APIKEY","value":""}]},"results":null}
```
...
```
curl https://thinkgridsoar.io/api/v1/workflows/{workflow_id}/executions/{execution_id}/abort -H "Authorization: Bearer APIKEY"
```
...
```
curl -XDELETE https://thinkgridsoar.io/api/v1/apps/{app_id} -H "Authorization: Bearer APIKEY"
```
...
```
curl https://thinkgridsoar.io/api/v1/apps/search -H "Authorization: Bearer APIKEY" -d '{"search": "APPNAME"}'
```
...
**PS: These are NOT accurate for https://thinkgridsoar.io yet.**
...
```
curl https://thinkgridsoar.io/api/v1/users -H "Authorization: Bearer APIKEY" 
```
...
```
curl https://thinkgridsoar.io/api/v1/users/register -H "Authorization: Bearer APIKEY" -d '{"username": "username", "password": "P@ssw0rd"}'
```
...
```
curl https://thinkgridsoar.io/api/v1/users/register -H "Authorization: Bearer APIKEY" -d '{"user_id": "USERID", "role": "user"}'
```
...
```
curl -XDELETE https://thinkgridsoar.io/api/v1/users/{userid} -H "Authorization: Bearer APIKEY" 
```
...
```
curl https://thinkgridsoar.io/api/v1/users/generateapikey -H "Authorization: Bearer APIKEY" -d '{"user_id": "id"}
```
